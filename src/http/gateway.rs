...
async fn handle_socket(
    ...
    let Some(identity) = receive_identity_message(&mut socket)
        ...
        tracing::error!("failed to get identity message from gateway socket");
        return;
    };
...
async fn task_send(
    ...
    loop {
        // TODO: receive from a channel
        let event: GatewayServerEvent = Default::default();
